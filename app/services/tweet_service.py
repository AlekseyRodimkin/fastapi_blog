...
async def tweet_by_id(tweet_id: int, db: AsyncSession = Depends(get_db), user_id=None) -> User | None:
    if user_id:
        tweet_result = await db.execute(select(Tweet).where(Tweet.id == tweet_id).where(Tweet.user_id == user_id))
    else:
...
async def tweets_by_user_ids(db: AsyncSession, user_ids: list[int], limit: int, offset: int) -> Sequence[Tweet]:
